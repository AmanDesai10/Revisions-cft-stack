...
import { getPostData, getOrganisationData } from "@/lib/fetchers";
...
export async function generateMetadata({
  ...
  const [data, organisationData] = await Promise.all([
  ...
  return {
    ...
    ...(params.domain.endsWith(`.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}`) &&
      organisationData.customDomain && {
      alternates: {
        canonical: `https://${organisationData.customDomain}/${params.slug}`,
      },
    }),
  };
}
    ...
    <>
      <div className="flex flex-col items-center justify-center">
        ...
        <a
          ...
          href={
            data.organisation?.user?.username
              ? `https://twitter.com/${data.organisation.user.username}`
              : `https://github.com/${data.organisation?.user?.gh_username}`
          }
          rel="noreferrer"
          target="_blank"
        >
          <div className="my-8">
            <div className="relative inline-block h-8 w-8 overflow-hidden rounded-full align-middle md:h-12 md:w-12">
              {data.organisation?.user?.image ? (
                <BlurImage
                  alt={data.organisation?.user?.name ?? "User Avatar"}
                  height={80}
                  src={data.organisation.user.image}
                  width={80}
                />
              ) : (
      ...
      <div className="m-auto w-3/4 w-full">
